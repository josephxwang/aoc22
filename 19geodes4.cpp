...
void rec_geodes(int& count, int time, vector<int> robots, vector<int> minerals, vector<vector<int>>& blueprint, int& max_geode) {
    ...
    // Try to buy each robot, starting from geode robot to clay robot
    for (int rob_i = robots.size()-1; rob_i >= 0; rob_i--) {
        ...
        // Buy robot
        if (can_buy) {
            // Don't buy ore robot past three-quarters point
            if (rob_i == 0 and time <= 24 * 3 / 4 - 1) {
                continue;
            }
            

            // // Don't buy obsidian robot past two-thirds point
            // if (rob_i == 1 and time <= 24 * 2 / 3 - 1) {
            //     continue;
            // }
