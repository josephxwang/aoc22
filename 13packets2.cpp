...
// Return index of next number or '[', where the recursive function should start
...
// Return index of next ',' or ']'
int findNextComma(int i, string s) {
    i++;
    while (s[i] != ',' and s[i] != ']') {
        i++;
    }
...
bool recPair(int& i, int& j, string& p1, string& p2) {
    ...
    // If p1 is a list and p2 isn't
        ...
        // Convert the other packet, adding brackets
        int numStart = j;
        int numEnd = findNextComma(j, p2);
        p2 = p2.substr(0, numStart) + '[' + p2.substr(numStart, numEnd-numStart) + ']' + p2.substr(numEnd);

        i++;
        j++;
        return recPair(i, j, p1, p2);
    }
    // If p1 isn't a list and p2 is
    else if ((p1[i] != '[' && p2[j] == '[')) {) {
        // Convert the other packet, adding brackets
        int numStart = i;
        int numEnd = findNextComma(i, p1);
        p1 = p1.substr(0, numStart) + '[' + p1.substr(numStart, numEnd-numStart) + ']' + p1.substr(numEnd);
