    ...
    while (getline(infile, line)) {
        ...
        // If zero node (no flow rate)
            ...
            for (auto v : zero_nodes_to_resolve) {
                ...
                if (zeros_graph.find(v) != zeros_graph.end()) {
                    // Find edge that contains u to resolve it
                    for (int i = 0; i < zeros_graph[v].size(); i++) {
                        if (u == zeros_graph[v][i].dest) {
                            // Find edge to resolve to (1 of 2)
                            for (edge e : zeros_graph[u]) {
                                // Check if unique edge
                                bool has_same_edge = false;
                                string common_node;

                                for (edge f : zeros_graph[v]) {
                                    if (e.dest == f.dest) {
                                        has_same_edge = true;
                                        common_node = e.dest;
                                    }
                                ...
                                if (has_same_edge) {
                                    for (int j = 0; j < graph[v].size(); j++) {
                                        if (u == graph[common_node][j].dest) {
                                            graph[common_node][j].dest = zeros_graph[u]
                                        }
                                    }
                                    continue;
                                }
                                ...
                                // Resolve proper edge (non-zero node)
                                zeros_graph[v][i].dest = e.dest;
                                zeros_graph[v][i].weight = e.weight + 1;
                                break;
                            }
